                    ...
                    <tbody>
                      @foreach ($data as $row)
                        <tr>
                          ...
                          <td class="text-center">
                            <a href="javascript:void(0)" id="approval{{ $row->employee_id }}" data-toggle="tooltip" data-id="{{ $row->employee_id }}" class="badge {{ $row->status === 'Approved' ? 'bg-success' : ( $row->status=='Sendback' ? 'bg-secondary' : 'bg-warning' ) }} rounded-pil px-1">{{ $row->status == 'Pending' ? 'Waiting For Approval' : ( $row->status=='Sendback'? 'Waiting For Revision' : $row->status) }}</a>
                          </td>
